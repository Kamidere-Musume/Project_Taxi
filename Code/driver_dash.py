...
#class driver_dash(tk.Frame):
   ...
   #def __init__(self,parent,controller):
      ...
      self.frm = tk.Frame(canvas, width=700,height=500,background="bisque")
      self.frm.place(relx=.5,rely=.7, anchor="c")
      self.book_tbl = ttk.Treeview(self.frm, columns=(1,2,3,4,5,6,7,8,9), show="headings",height="10")
      self.book_tbl.pack()
      
      # Adding scrollbar in tree view 
      self.scroll = ttk.Scrollbar(self.frm,orient="vertical",command= self.book_tbl.yview)
      self.scroll.place(x=30+200+2, y = 95, height=200+20)
      ...
      self.book_tbl.column(3,stretch = NO,width=120)      
      self.book_tbl.heading(3,text="User First Name")
      
      self.book_tbl.column(4,stretch = NO,width=120)      
      self.book_tbl.heading(4,text="User Last Name")
      
      self.book_tbl.column(5,stretch = NO,width=130)      
      self.book_tbl.heading(5,text="User Number")
      
      self.book_tbl.column(6,stretch = NO,width=120)      
      self.book_tbl.heading(6,text="Pick up")
      
      self.book_tbl.column(7,stretch = NO,width=120)
      self.book_tbl.heading(7,text="Destination")
      
      self.book_tbl.column(8,stretch = NO,width=120)      
      self.book_tbl.heading(8,text="Price")
      
      self.book_tbl.column(9,stretch = NO,width=120)
      self.book_tbl.heading(9,text="Distance")

      # Creating Button 
      show_btn  = tk.Button(canvas, text="Show Requests",command=self.showtable)
      show_btn.place(x=800, y=400, height=25, width=90)

      
   def showtable(self):
      con,cur = dbcon()
      query = "select booking.Booking_Id,user.User_Id,user.User_fname, user.User_lname, user.User_phone_Number,booking.Current_location,booking.Destination,booking.Price,booking.Distance From booking inner join user on User.User_Id = booking.User_Id;"
      cur.execute(query)
      rows = cur.fetchall()
      con.commit()
      cur.close()
